    ...
    def constraints(self, currentIndexVariable, varArray, varDomain):
        for j in range(len(varArray)):
            if (self.assigned[j] == 1):
                if ((abs(varArray[j] - varArray[currentIndexVariable]) == abs(j - currentIndexVariable)) or (
                        varArray[j] == varArray[currentIndexVariable])):
                    return False
        return True
...
# test
a = [[0, 1, 2, 3, 4, 5, 6, 7], [0, 1, 2, 3, 4, 5, 6, 7], [0, 1, 2, 3, 4, 5, 6, 7], [0, 1, 2, 3, 4, 5, 6, 7],
